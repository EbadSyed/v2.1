...
function GetInfo(infoObject) {
    infoObject.Name = "Silicon Labs - IAR Project Generator ";
    infoObject.Description = "Generates IAR application project from standard Bluetooth Profiles and Services for Blue Geckos";
...
function RunPlugin(profiledata)
{
  ...
  log("5. Generating IAR project files");
...
{
  // Copying ble_workspace.eww file
  var fileName = "ble_workspace.eww";
  log("Copying " + fileName);
  FileManager.CopyFile(fileName, fileName);

  // Copying ble_workspace.custom_argvars file
  var fileName = "ble_workspace.custom_argvars";
  log("Copying " + fileName);
  FileManager.CopyFile(fileName, fileName);

  // Copying ble_device.ewd file
  var fileName = "ble_device.ewd";
  log("Copying " + fileName);
  FileManager.CopyFile(fileName, "iar/" + fileName);

  // Generating ble_device.ewp file
  var ewpTemplate = FileManager.ReadFile("ble_device.ewp.template");
  var ewpSource = ProcessTemplate(ewpTemplate, profiledata);
  var ewpFileName = "ble_device.ewp";
  log("Generating "+ewpFileName);
  FileManager.CreateFile("iar/" + ewpFileName, ewpSource);

  // Copying bleDemo.icf file
  var fileName = "bleDemo.icf";
  log("Copying " + fileName);
  FileManager.CopyFile(fileName, "iar/" + fileName);
}
