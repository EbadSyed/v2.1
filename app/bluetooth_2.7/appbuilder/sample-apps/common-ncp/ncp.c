...
static uint32_t (*transmit_callback)(uint8* data, uint8 len) = 0;
...
void ncp_set_transmit_callback(uint32_t (*_transmit_callback)(uint8* data, uint8 len))
{
...
void ncp_transmit_enqueue(struct gecko_cmd_packet *evt)
{
  if (evt == NULL) {
    return;
  }
  if (!ncp_enqueue(tx_queue, (uint8_t*)evt, BGLIB_MSG_LEN(evt->header) + BGLIB_MSG_HEADER_LEN)) {
    EFM_ASSERT(false);
