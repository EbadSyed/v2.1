...
def getPlatform():
  ...
  # *cygwin* --> windows
    ...
    print "platform.system().lower(): %s" % detOs
    detOs = supportedPlatforms[0]
    print "supportedPlatforms[0]: %s" % detOs
...
def getCodegenContext():
  ...
  configData = []
  configDir = os.path.join(scriptDir, configDir)
  configFiles = os.listdir(configDir)
  for configFile in configFiles:
    if configFile.endswith(".json"):
      configData.append(json.loads(open(os.path.join(configDir, configFile), "r").read()))
  return configData
...
def sanityCheckConfiguration(codegenContext, sampleApp=None):
  ...
  # check for code generator module id and group id duplicates
  cgmIds = []
  for configModule in codegenContext:
    if "codeGenModules" in configModule:
      for cgm in configModule["codeGenModules"]:
        if cgm["id"] in cgmIds:
          return generatorError("Error: duplicate CGM: '%s'!" \
                                % cgm["id"])
        cgmIds.append(cgm["id"])
        if "groups" in cgm:
          cgmgIds = []
          for cgmg in cgm["groups"]:
  ...
  for cgKey in cgKeys:
    ...
    for configModule in codegenContext:
      if cgKey in configModule:
        if foundKey:
          return generatorError("Error: duplicate '%s' key definition!" % cgKey)
        else:
          foundKey = True
...
def setupGenerateStudio(context, verbose):
  ...
  codeGenRoot = ""
  result = []
  # get root directory if codegen; sanity check guarantees that we find it
  for configModule in context["CODEGEN"]:
    if "codeGenRoot" in configModule:
      codeGenRoot = configModule["codeGenRoot"]
      break
  # find sample app
  for configModule in context["CODEGEN"]:
...
def name():
  ...
  Returns:
      Name of generator.
  """
  return "BLE SDK External Generator"
...
def searchPathForModules():
  ...
  """
  return ["../../../tool/code_generator/modules"]
