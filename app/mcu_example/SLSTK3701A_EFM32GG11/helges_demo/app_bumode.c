             ...
             uint8_t * currentApp)
{
  ...
  // Turn on RTCC clocks so we can read from its registers
  ...
  EMU->BUCTRL = EMU_BUCTRL_EN | EMU_BUCTRL_PWRRES_RES3 | EMU_BUCTRL_BUVINPROBEEN;
...
void buPwrSwitch(bool chargeEn, bool disCharge)
{
  if (chargeEn) {
    ...
    EMU->BUCTRL = EMU_BUCTRL_EN | EMU_BUCTRL_BUINACTPWRCON_MAINBU | EMU_BUCTRL_PWRRES_RES3 | EMU_BUCTRL_BUVINPROBEEN;
  } else if (disCharge) {
    // Ground backup battery to discharge it
    EMU->BUCTRL = EMU_BUCTRL_EN | EMU_BUCTRL_BUVINPROBEEN;
