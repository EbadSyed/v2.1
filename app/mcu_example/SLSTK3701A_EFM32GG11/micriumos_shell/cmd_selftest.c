                       ...
                       SHELL_CMD_PARAM *pcmd_param)
{
  ...
  for (i = 1; i < argc; i++) {
    if (!Str_Cmp(argv[i], "-help")) {
      printf("selftest - Checks if interrupt vector table is within program memory.\n");
      return SHELL_EXEC_ERR_NONE;
    } else {
      printf("selftestCmd: Invalid argument '%s'.\n", argv[i]);
  ...
  for (i = VTOROFFSET; i < EXT_IRQ_COUNT + VTOROFFSET; i++) {
    if ( (uint32_t)(((uint32_t*)SCB->VTOR)[i] - FLASH_BASE) > FLASH_SIZE ) {
      printf("IRQ %s out of program memory\n", getIrqName((IRQn_Type)(i - VTOROFFSET)));
      errorCounter++;
    }
  }

  if (errorCounter == 0) {
    printf("All the interrupt pointers are within the program memory\n");
  }
