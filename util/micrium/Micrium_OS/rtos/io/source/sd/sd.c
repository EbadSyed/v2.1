...
#include  <rtos/io/source/sd/sd_priv.h>
#include  <rtos/io/source/sd/sd_io_fnct_priv.h>
#include  <rtos/io/source/sd/sd_mem_priv.h>
...
#if (RTOS_CFG_EXTERNALIZE_OPTIONAL_CFG_EN == DEF_DISABLED)
void  SD_ConfigureMemSegPtr (MEM_SEG  *p_seg,
                             MEM_SEG  *p_seg_buf)
{
...
void  SD_Init (RTOS_ERR  *p_err)
{
    ...
    p_sd->EventHeadPtr         = DEF_NULL;
    p_sd->EventTailPtr         = DEF_NULL;
    ...
    RTOS_ASSERT_CRITICAL(RTOS_ERR_CODE_GET(*p_err) == RTOS_ERR_NONE, RTOS_ERR_ASSERT_CRITICAL_FAIL, ;);
                                                                ...
                                                                /* Create core task and semaphore.                      */
    SD_Ptr->EventSemHandle = KAL_SemCreate("IO - SD event sem",
                                            DEF_NULL,
                                            p_err);
    RTOS_ASSERT_CRITICAL(RTOS_ERR_CODE_GET(*p_err) == RTOS_ERR_NONE, RTOS_ERR_ASSERT_CRITICAL_FAIL, ;);
...
*
* @brief    Report card detect event.
*
* @param    bus_handle      Handle to SD bus.
*********************************************************************************************************
*/
...
*
* @brief    Report card remove event.
*
* @param    bus_handle      Handle to SD bus.
*********************************************************************************************************
*/
...
void  SD_EventPost (SD_BUS_HANDLE  bus_handle,
                    ...
                    RTOS_ERR       err)
{
    ...
    p_event = (event_type == SD_EVENT_TYPE_DATA_XFER_CMPL) ? SD_PreAllocEventGet() :
                                                             &bus_handle->BusEvent;
    ...
    if (SD_Ptr->EventTailPtr != DEF_NULL) {
        SD_Ptr->EventTailPtr->NextPtr = p_event;
    }
    SD_Ptr->EventTailPtr = p_event;

    if (SD_Ptr->EventHeadPtr == DEF_NULL) {
        SD_Ptr->EventHeadPtr = SD_Ptr->EventTailPtr;
    }
    CPU_CRITICAL_EXIT();

    KAL_SemPost(SD_Ptr->EventSemHandle,
                KAL_OPT_POST_NONE,
               &err_local);
...
static  void  SD_CoreTaskHandler (void  *p_data)
{
    ...
    while (DEF_ON) {
        ...
        KAL_SemPend(SD_Ptr->EventSemHandle,
                    KAL_OPT_NONE,
        ...
        CPU_CRITICAL_ENTER();
        p_event              =  SD_Ptr->EventHeadPtr;
        event                = *p_event;
        SD_Ptr->EventHeadPtr = (SD_EVENT *)event.NextPtr;

        if (SD_Ptr->EventHeadPtr == DEF_NULL) {
            SD_Ptr->EventTailPtr =  DEF_NULL;
        }
        CPU_CRITICAL_EXIT();

        if (p_event != &event.BusHandle->BusEvent) {
        ...
        switch (event.Type) {
            case SD_EVENT_TYPE_CARD_DETECT:                     /* Card detected. Initialize it.                        */
                 event.BusHandle->CardPresent = DEF_YES;
