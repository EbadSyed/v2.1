...
*
*               (2) The signal MUST be blocked before locking the mutex to prevent a ISR signal to be handled
...
* Return(s)   : None.
*
* Note(s)     : (1) The Interrupt signal should not be blocked before calling this function since they will
...
* Return(s)   : None.
*
* Note(s)     : (1) The signal MUST be blocked before locking the mutex to prevent a ISR signal to be handled
*                   by this thread and having the mutex in an unknown state. The Mutex MUST then be released
*                   before unblocking the signals.
*********************************************************************************************************
*/
...
* Return(s)   : None.
*
* Note(s)     : (1) Signals are already disabled in this task to prevent an ISR to be handled within this
*                   thread.
*
*               (2) Because signals are always disbaled, every Interrupt should be called with the Internal
