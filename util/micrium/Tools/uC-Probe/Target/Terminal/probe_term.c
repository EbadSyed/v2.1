...
PROBE_INT08U  const  ProbeTermBufSize = PROBE_TERM_CFG_BUF_SIZE;
...
typedef struct  probe_term_states {                             /* --------- UC/PROBE TERMINAL WINDOW STATES ---------- */
    PROBE_BOOLEAN      RxState;                                 /* Current state of the Reception    state machine.     */
    PROBE_BOOLEAN      TxState;                                 /* Current state of the Transmission state machine.     */
    PROBE_INT08U       RxCtr;                                   /* Counter for Reception    timeout purposes.           */
    PROBE_INT08U       TxCtr;                                   /* Counter for Transmission timeout purposes.           */
} PROBE_TERM_STATES;
...
typedef struct  probe_term_q_msg  PROBE_TERM_Q_MSG;
    ...
    PROBE_CHAR         Msg[PROBE_TERM_CFG_BUF_SIZE];
    PROBE_INT08U       MsgLen;
...
typedef struct  probe_term_q {
    ...
    PROBE_INT32U       MsgCtr;                                  /* Number of messages in the queue.                     */
} PROBE_TERM_Q;
...
PROBE_INT08U       ProbeTermTimeoutCnt;                         /* Number of times a flag should be read until it gets  */
                                                                /* raised by uC/Probe.                                  */
...
static  PROBE_TERM_Q_MSG  *ProbeTermQ_GetSlot (PROBE_INT08U       msg_type);

static  void               ProbeTermQ_PutMsg  (PROBE_INT08U       msg_type,
                                               PROBE_TERM_Q_MSG  *p_msg);

static  PROBE_TERM_ERR     ProbeTermQ_GetMsg  (PROBE_INT08U       msg_type);

static  void               ProbeTermQ_FreeMsg (PROBE_INT08U       msg_type,
                                               PROBE_TERM_Q_MSG  *p_msg);
...
#if PROBE_TERM_CFG_CMD_EN > 0
PROBE_TERM_ERR  ProbeTermCmdPrint (char      *p_str)
{
    PROBE_TERM_ERR     err;
    PROBE_TERM_Q_MSG  *p_msg;
    PROBE_INT16U       offset;
    PROBE_INT08U       chunk_cnt;
    PROBE_INT08U       chunk_ix;
...
#if PROBE_TERM_CFG_TRC_EN > 0
PROBE_TERM_ERR  ProbeTermTrcPrint (char      *p_str)
{
    PROBE_TERM_ERR     err;
    PROBE_TERM_Q_MSG  *p_msg;
    PROBE_INT16U       offset;
...
void  ProbeTermQ_Init (PROBE_TERM_Q *p_q, PROBE_INT08U max)
{
    PROBE_TERM_Q_MSG  *p_msg;
    PROBE_INT08U       ix;
...
static  PROBE_TERM_Q  *ProbeTermQ_GetRef (PROBE_INT08U msg_type)
{
    PROBE_TERM_Q  *p_q;
