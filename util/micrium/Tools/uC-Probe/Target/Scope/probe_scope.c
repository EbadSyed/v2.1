...
struct  scope_ch {
    PROBE_BOOLEAN          En;                                                // Channel enabled (1) or disabled (0)
    PROBE_INT08U           DataType;                                          // Source data type: 
                                                                              //    0 = PROBE_INT08U
                                                                              //    1 = PROBE_INT08S
                                                                              //    2 = PROBE_INT16U  (only if 'PROBE_SCOPE_16_BIT_EN' is set to 1)
                                                                              //    3 = PROBE_INT16S  (only if 'PROBE_SCOPE_16_BIT_EN' is set to 1)
                                                                              //    4 = PROBE_INT32U  (only if 'PROBE_SCOPE_32_BIT_EN' is set to 1)
                                                                              //    5 = PROBE_INT32S  (only if 'PROBE_SCOPE_32_BIT_EN' is set to 1)
                                                                              //    6 = PROBE_FP32    (only if 'PROBE_SCOPE_32_BIT_EN' is set to 1)
    PROBE_BOOLEAN           BitEn;                                            // Bit mode enabled (1) or disabled (0)
    PROBE_INT08U            BitSel;                                           // Selected bit in bit mode (0..63)
    PROBE_INT08U           *DataAddr;                                         // Address of source data (i.e. address of symbol assigned to channel) 
...
volatile  PROBE_INT32U         ProbeScope_DataRdyFlag;                        // Data is ready to be read by Probe

volatile  PROBE_BOOLEAN        ProbeScope_InitFlag;                           // Flag indicating MCU has reset

static    PROBE_INT08U         ProbeScope_Mode;                               // Scope Mode:
                                                                              //     0 = Stop
                                                                              //     1 = Single
                                                                              //     2 = Continuous
                                                                              //     3 = Triggered (always assumes pre-triggering)
                                                                            
static    PROBE_INT08U         ProbeScope_TrigChSel;                          // Channel that will be monitored for the trigger:
                                                                              //     1 = Channel #1
                                                                              //     2 = Channel #2
                                                                              //     3 = Channel #3
                                                                              //     4 = Channel #4
...
volatile  PROBE_INT32U         ProbeScope_TrigDispPos;                        // Position of the trigger on the screen (in sample number):
                                                                              //     0                           = Far left,  identical to 100% post-triggering
                                                                              //     PROBE_SCOPE_MAX_SAMPLES - 1 = Far right, identical to 100% pre-triggering
volatile  PROBE_BOOLEAN        ProbeScope_TrigFlag;                           // Flag indicating that we satisfied the trigger
volatile  PROBE_INT32U         ProbeScope_TrigHoldOff;                        // Determines how many samples we will hold off before retriggering
static    PROBE_INT32U         ProbeScope_TrigHoldOffCtr;
volatile  PROBE_INT32U         ProbeScope_TrigIx;                             // Position in the sample buffer where the trigger was found
static    PROBE_BOOLEAN        ProbeScope_TrigSlope;                          // Trigger Slope
                                                                              //     0 = Low  going
                                                                              //     1 = High going

static    PROBE_INT32U         ProbeScope_SampleIxPrev;
static    PROBE_INT32U         ProbeScope_SampleIxCur;
static    PROBE_INT32U         ProbeScope_SamplesPreTrigCtr;                  // Minimum number of samples needed before we are allowed to trigger
static    PROBE_INT32U         ProbeScope_SamplesPostTrigCtr;                 // Determines how many samples will be take after a trigger has been detected:
...
static    PROBE_INT16U         ProbeScope_SamplingClkDiv;                     // Clk source divider (1 to 1000)
static    PROBE_INT16U         ProbeScope_SamplingClkDivCtr; 
volatile  PROBE_INT32U         ProbeScope_SamplingClkHz;                      // Sampling Clock Frequency in Hz

static    PROBE_INT08U         ProbeScope_State;                              // Scope State
...
static  volatile  PROBE_INT16U   ProbeScopeDbg_MaxSamples     = PROBE_SCOPE_MAX_SAMPLES;
static  volatile  PROBE_INT08U   ProbeScopeDbg_MaxCh          = PROBE_SCOPE_MAX_CH;
static  volatile  PROBE_BOOLEAN  ProbeScopeDbg_16BitEn        = PROBE_SCOPE_16_BIT_EN;
static  volatile  PROBE_BOOLEAN  ProbeScopeDbg_32BitEn        = PROBE_SCOPE_32_BIT_EN;
    ...
    if (p_ch->En == 1) {
        ...
        switch (p_ch->DataType) {
            case PROBE_SCOPE_INT08U:
                 if (p_ch->BitEn == 0) {
                     p_sample->Val08U     = *(PROBE_INT08U *)p_ch->DataAddr;
                 } else {
                     msk.Val08U           =  (PROBE_INT08U)1 << p_ch->BitSel;
                     if (*(PROBE_INT08U *)p_ch->DataAddr & msk.Val08U) {
                         p_sample->Val08U = 1;
                     } else {
                         p_sample->Val08U = 0;
                     }
            ...
            case PROBE_SCOPE_INT08S:
                 if (p_ch->BitEn == 0) {
                     p_sample->Val08S     = *(PROBE_INT08S *)p_ch->DataAddr;
                 } else {
                     msk.Val08S           =  (PROBE_INT08S)1 << p_ch->BitSel;
                     if (*(PROBE_INT08S *)p_ch->DataAddr & msk.Val08S) {
                         p_sample->Val08S = 1;
                     } else {
                         p_sample->Val08S = 0;
                     }
            ...
            case PROBE_SCOPE_INT16U:
                 if (p_ch->BitEn == 0) {
                     p_sample->Val16U     = *(PROBE_INT16U *)p_ch->DataAddr;
                 } else {
                     msk.Val16U           =  (PROBE_INT16U)1 << p_ch->BitSel;
                     if (*(PROBE_INT16U *)p_ch->DataAddr & msk.Val16U) {
                         p_sample->Val16U = 1;
                     } else {
                         p_sample->Val16U = 0;
                     }
            ...
            case PROBE_SCOPE_INT16S:
                 if (p_ch->BitEn == 0) {
                     p_sample->Val16S     = *(PROBE_INT16S *)p_ch->DataAddr;
                 } else {
                     msk.Val16S           =  (PROBE_INT16S)1 << p_ch->BitSel;
                     if (*(PROBE_INT16S *)p_ch->DataAddr & msk.Val16S) {
                         p_sample->Val16S = 1;
                     } else {
                         p_sample->Val16S = 0;
                     }
            ...
            case PROBE_SCOPE_INT32U:
                 if (p_ch->BitEn == 0) {
                     p_sample->Val32U     = *(PROBE_INT32U *)p_ch->DataAddr;
                 } else {
                     msk.Val32U           =  (PROBE_INT32U)1 << p_ch->BitSel;
                     if (*(PROBE_INT32U *)p_ch->DataAddr & msk.Val32U) {
                         p_sample->Val32U = 1;
                     } else {
                         p_sample->Val32U = 0;
                     }
            ...
            case PROBE_SCOPE_INT32S:
                 if (p_ch->BitEn == 0) {
                     p_sample->Val32S     = *(PROBE_INT32S *)p_ch->DataAddr;
                 } else {
                     msk.Val32S           =  (PROBE_INT32S)1 << p_ch->BitSel;
                     if (*(PROBE_INT32S *)p_ch->DataAddr & msk.Val32S) {
                         p_sample->Val32S = 1;
                     } else {
                         p_sample->Val32S = 0;
                     }
            ...
            case PROBE_SCOPE_FP32:
                 p_sample->ValFP32  = *(PROBE_FP32   *)p_ch->DataAddr;
                 break;
#endif
        }                          
    }
