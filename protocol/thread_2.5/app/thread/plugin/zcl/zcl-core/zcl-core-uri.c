...
// "length" is the number of characters to read.  Strings with any
// characters not in [0-9a-fA-F are rejected as invalid.

bool emHexStringToInt(const uint8_t *chars, size_t length, uintmax_t *result)
{
  for (; *chars == '0' && 0 < length; chars++, length--) {
  }
...
// This used in parsing URIs, where leading zeros are not allowed,
// so it doesn't allow them.
//   "01": returns false
// Otherwise it behaves as emHexStringToInt().
bool emZclHexStringToInt(const uint8_t *chars, size_t length, uintmax_t *result)
{
  if (length == 0
    ...
    return emHexStringToInt(chars, length, result);
  }
}
...
static int decodeBase64Url(const uint8_t *input,
                           size_t inputLength,
                           uint8_t *output,
                           size_t outputLength)
{
                         ...
                         uint16_t *resultBits)
{
  ...
  int byteCount = decodeBase64Url(base64Url,
                                  length,
                                  result->bytes,
                                  sizeof(result->bytes));
...
bool emZclNiUriToUid(const uint8_t *uri, uint16_t uriLength, EmberZclUid_t *uid)
{
  return (niPrefixLength + EMBER_ZCL_UID_BASE64URL_LENGTH <= uriLength
          && strncmp(uri, niPrefix, niPrefixLength) == 0
          && 0 < decodeBase64Url(uri + niPrefixLength,
