...
EmZclUriPath emZclWellKnownUriPaths[] = {
  // .well-known/core
  {   1, 255, EM_ZCL_URI_FLAG_METHOD_GET | EM_ZCL_URI_FLAG_FORMAT_LINK, emZclUriPathStringMatch, ".well-known", NULL, NULL },
  { 255, 255, EM_ZCL_URI_FLAG_METHOD_GET | EM_ZCL_URI_FLAG_FORMAT_LINK, emZclUriPathStringMatch, "core", wellKnownUriQueries, wellKnownUriHandler },
};
...
} DiscPayloadContextStatus;

typedef struct DiscPayloadContext_s DiscPayloadContext_t;
struct DiscPayloadContext_s {
  EmberCoapContentFormatType contentFormat;
  DiscPayloadContextStatus status;
  CborState cborState; // reuse start/finger/end pointers for non-CBOR buffer implementation
  uint8_t linkCount;
  uint8_t attrCount;

  bool (*payloadIsEmpty)(DiscPayloadContext_t *dpc);
  const uint8_t *(*payloadPointer)(DiscPayloadContext_t *dpc);
  uint16_t (*payloadLength)(DiscPayloadContext_t *dpc);
  void (*startPayload)(DiscPayloadContext_t *dpc);
  void (*startLink)(DiscPayloadContext_t *dpc);
  void (*addResourceUri)(DiscPayloadContext_t *dpc,
                         EmberZclEndpointId_t endpointId,
                         const EmberZclClusterSpec_t *clusterSpec);
  void (*addRt)(DiscPayloadContext_t *dpc,
                const EmberZclClusterSpec_t *spec,
                bool includeTrailingClusterTag);
  void (*addIf)(DiscPayloadContext_t *dpc,
                EmberZclEndpointId_t endpointId,
                const EmberZclClusterSpec_t *clusterSpec);
  void (*addEp)(DiscPayloadContext_t *dpc);
  void (*addZe)(DiscPayloadContext_t *dpc,
                EmberZclEndpointId_t endpointId,
                EmberZclDeviceId_t deviceId);
  void (*endLink)(DiscPayloadContext_t *dpc);
  void (*endPayload)(DiscPayloadContext_t *dpc);
  void (*appendUid)(DiscPayloadContext_t *dpc);
};

static bool initDiscPayloadContext(DiscPayloadContext_t *dpc,
                                   EmberCoapContentFormatType contentFormat,
                                   uint8_t *buffer,
                                                        ...
                                                        uint8_t depth)
{
  ...
  if (MEMCOMPARE(finger,
    ...
    char *zeOptionDot = memchr(finger,
                               EM_ZCL_URI_QUERY_DOT,
                               end - finger);
    char *wildcard = memchr(finger,
                            EM_ZCL_URI_QUERY_WILDCARD,
...
static void wellKnownUriHandler(EmZclContext_t *context)
{
  if (context->mask == 0) {
    if (isMulticastAddress(context->info->localAddress.bytes)) {
      ...
      emZclRespond205ContentLinkFormat((const uint8_t *)"", 0, EMBER_COAP_CONTENT_FORMAT_LINK_FORMAT_PLUS_CBOR);
    } else {
...
#define MAX_WELL_KNOWN_REPLY_PAYLOAD (560)
...
static void wellKnownUriQueryHandler(EmZclContext_t *context)
{
  ...
  EmberCoapContentFormatType accept = EMBER_COAP_CONTENT_FORMAT_LINK_FORMAT_PLUS_CBOR;
  ...
  DiscPayloadContext_t dpc;
  if (!initDiscPayloadContext(&dpc, accept, (uint8_t *)payload, MAX_WELL_KNOWN_REPLY_PAYLOAD)) {
  ...
  if ((context->mask & EM_ZCL_DISCOVERY_CONTEXT_QUERY_FOR_ZCLIP_SUPPORT_BY_CLUS)
      || (context->mask & EM_ZCL_DISCOVERY_CONTEXT_QUERY_FOR_ZCLIP_SUPPORT_BY_RESOURCE_VERSION)) {
  ...
  if (dpc.status == DISCOVERY_PAYLOAD_CONTEXT_STATUS_SUCCESS) {
    if (isMulticastAddress(context->info->localAddress.bytes) && dpc.payloadIsEmpty(&dpc)) {
      ...
      emZclRespond205ContentLinkFormat(dpc.payloadPointer(&dpc),
...
bool emberZclDiscSend(EmberCoapResponseHandler responseHandler)
{
  ...
  if (!emZclGetMulticastAddress(&allThreadNodes)) {
    emberAfAppPrintln("Unable to send discovery message!");
    return false;
  }
  ...
  EmberStatus status = emberCoapGet(&allThreadNodes,
...
static bool appendPayloadLinkFormat(DiscPayloadContext_t *dpc, uint8_t *data, uint16_t dataLength)
{
  if (dpc->status == DISCOVERY_PAYLOAD_CONTEXT_STATUS_SUCCESS) {
...
static void separateLinkLinkFormat(DiscPayloadContext_t *dpc)
{
  if (dpc->linkCount != 0) {
...
static void separateAttrLinkFormat(DiscPayloadContext_t *dpc)
{
  appendPayloadLinkFormat(dpc, (uint8_t *)";", 1);
}

static bool payloadIsEmptyLinkFormat(DiscPayloadContext_t *dpc)
{
  // Zero length.
  return (dpc->payloadLength(dpc) == 0);
}

static void startPayloadLinkFormat(DiscPayloadContext_t *dpc)
{
  // Nothing to do.
}

static void startLinkLinkFormat(DiscPayloadContext_t *dpc)
{
  separateLinkLinkFormat(dpc);
  dpc->linkCount++;
}

static void addResourceUriLinkFormat(DiscPayloadContext_t *dpc,
                                     EmberZclEndpointId_t endpointId,
                                     const EmberZclClusterSpec_t *clusterSpec)
{
...
static void addRtLinkFormat(DiscPayloadContext_t *dpc,
                            const EmberZclClusterSpec_t *spec,
                            bool includeTrailingClusterTag)
{
...
static void addIfLinkFormat(DiscPayloadContext_t *dpc,
                            EmberZclEndpointId_t endpointId,
                            const EmberZclClusterSpec_t *clusterSpec)
{
  separateAttrLinkFormat(dpc);
...
static void addEpLinkFormat(DiscPayloadContext_t *dpc)
{
  separateAttrLinkFormat(dpc);
...
static void addZeLinkFormat(DiscPayloadContext_t *dpc,
                            EmberZclEndpointId_t endpointId,
                            EmberZclDeviceId_t deviceId)
{
  separateAttrLinkFormat(dpc);
...
static void endLinkLinkFormat(DiscPayloadContext_t *dpc)
{
  dpc->linkCount++;
}

static void endPayloadLinkFormat(DiscPayloadContext_t *dpc)
{
  // Nothing to do.
}
...
static bool payloadIsEmptyLinkFormatPlusCbor(DiscPayloadContext_t *dpc)
{
  // Either zero length, or first byte signifies empty CBOR array.
  return (dpc->payloadLength(dpc) == 0 || *(dpc->payloadPointer(dpc)) == 0x80);
}

static void startPayloadLinkFormatPlusCbor(DiscPayloadContext_t *dpc)
{
  // Open CBOR array.
...
static void startLinkLinkFormatPlusCbor(DiscPayloadContext_t *dpc)
{
  // Open CBOR map.
...
static void addResourceUriLinkFormatPlusCbor(DiscPayloadContext_t *dpc,
                                             EmberZclEndpointId_t endpointId,
                                             const EmberZclClusterSpec_t *clusterSpec)
{
  // Construct value string.
...
static void addRtLinkFormatPlusCbor(DiscPayloadContext_t *dpc,
                                    const EmberZclClusterSpec_t *spec,
                                    bool includeTrailingClusterTag)
{
  // Construct value string.
...
static void addIfLinkFormatPlusCbor(DiscPayloadContext_t *dpc,
                                    EmberZclEndpointId_t endpointId,
                                    const EmberZclClusterSpec_t *clusterSpec)
{
  // Construct value string.
...
static void addEpLinkFormatPlusCbor(DiscPayloadContext_t *dpc)
{
  // Construct value string.
  char buffer[EM_ZCL_URI_DEVICE_UID_APPEND_MAX_LEN] = { 0 };
...
static void addZeLinkFormatPlusCbor(DiscPayloadContext_t *dpc,
                                    EmberZclEndpointId_t endpointId,
                                    EmberZclDeviceId_t deviceId)
{
  // Construct value string.
...
static void endLinkLinkFormatPlusCbor(DiscPayloadContext_t *dpc)
{
  // Close CBOR map.
...
static void endPayloadLinkFormatPlusCbor(DiscPayloadContext_t *dpc)
{
  // Close CBOR array.
  if (!emCborEncodeBreak(&dpc->cborState)) {
    dpc->status = DISCOVERY_PAYLOAD_CONTEXT_STATUS_BUFFER_OVERFLOW;
...
static const uint8_t *payloadPointerCommon(DiscPayloadContext_t *dpc)
{
  return dpc->cborState.start;
}

static uint16_t payloadLengthCommon(DiscPayloadContext_t *dpc)
{
  return emCborEncodeSize(&dpc->cborState);
}

static void appendUidCommon(DiscPayloadContext_t *dpc)
{
  dpc->startLink(dpc);
                                   ...
                                   uint16_t bufferLength)
{
  ...
  MEMSET(dpc, 0, sizeof(DiscPayloadContext_t));
