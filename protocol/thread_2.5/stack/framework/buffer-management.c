...
//  fprintf(stderr, "[start]\n");
  ...
  do {
    ...
    ATOMIC(
      ...
      if (b != NULL_BUFFER) {
        uint16_t *ptr = expandPointerNoCheck(b);
        if (ptr != NULL) {
          clearPhyFreelistBit(ptr);
        }
...
bool emPhyToMacQueueAddIsr(uint8_t *packet, uint8_t length, uint8_t padding)
{
  ...
  if (0 < phyFreelistCount) {
    ...
    if (tracePhase == NOT_TRACING) {
      ...
      uint16_t *ptr = expandPointer(rxPayload);
      if (ptr != NULL) {
        clearPhyFreelistBit(ptr);
      }
    } else {
...
{
  uint8_t *bufferPointer = emGetBufferPointer(buffer);
  if (bufferPointer != NULL) {
    bufferPointer += index;
